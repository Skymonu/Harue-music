...
def start_pannel():
    buttons = [
        ...
        [
            InlineKeyboardButton("📚 ᴄᴏᴍᴍᴀɴᴅ​ 📚", url="https://telegra.ph/file/072d2ef71286b8719c84b.jpg"),
        ],
        [
            InlineKeyboardButton("🌐 sᴏᴜʀᴄᴇ ᴄᴏᴅᴇ 🌐", url="https://github.com/Skymonu/Harue-music"),
        ],
    ]
    return (
        ...
        try:
            if member.id in OWNER:
                return await message.reply_text(
                    f"💡 Tuhan saya [{member.mention}] baru saja bergabung di grup ini."
                )
            if member.id in SUDOERS:
                return await message.reply_text(
                    f"💡 Pengurus saya [{member.mention}] baru saja bergabung di grup ini."
                )
            if member.id == ASSID:
            ...
            if member.id == BOT_ID:
                ...
                await message.reply_text(
                    f"""
👋 ** Halo senang rasanya bisa bergabung di grup ini kontoI**
